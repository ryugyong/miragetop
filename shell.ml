...
module Shell (DATA: Mirage_types_lwt.KV_RW) = struct
  ...
  let prepare ppf =
    ...
    try
      let res =
        List.for_all (load_file ppf) (List.rev !preload_objects)
      in
      !Loop.toplevel_startup_hook ();
      res
    with x ->
  ...
  let init data =
    mtop_read_key := (fun k ->
      Lwt_main.run (DATA.get data @@ Mirage_kv.Key.v k >>= function
                    | Error _e -> Lwt.return "not found"
                    | Ok s -> Lwt.return s));
  ...
  let eval_to_string key =
    let b = Buffer.create 0 in
    std_out := Format.formatter_of_buffer b;
    eval key !std_out;
    Buffer.contents b
